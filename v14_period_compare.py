"""
V14 PERIOD ROBUSTNESS — Strategy vs Buy&Hold across time windows
=================================================================
Runs the v14 engine (A/B-selected config) over:
  1. Fixed lookback windows ending at the last candle (3M..36M, Full)
...
v13 is included on the fixed windows for reference.

Usage:   .venv/bin/python v14_period_compare.py
Outputs: console tables + data/v14_period_*.csv + v14_period_chart.png
"""
import os
import numpy as np
import pandas as pd
...
from v14 import (load_real_data, add_indicators, run_backtest,
                 calculate_metrics, make_cfg, TRAIL_OVERRIDES_V14, WARMUP)
...
# The A/B-selected configuration (P2: core + pyramiding, adaptive OFF)
...
def load_all():
    ...
    for name, f in FILES.items():
        dfs[name] = add_indicators(load_real_data(f))
    return dfs
...
def eval_window(name, df_full, start_ts, end_ts=None, cfg=None):
    """Fresh-capital run on ONE window -> metric row (None if invalid)."""
    ...
    if eval_start > start_ts + pd.Timedelta(days=7):
        return None
    tr, eq, bh = run_backtest(sub, cfg or V14_SELECTED, INITIAL_CAPITAL,
...
def run_fixed(dfs, include_v13=True):
    rows = []
    v13_mod = None
    if include_v13:
        try:
            import main as v13_mod
        except Exception:
            v13_mod = None
    for name, df in dfs.items():
        ...
        for label, start in wins:
            m = eval_window(name, df, start, None, V14_SELECTED)
            if m:
                m['Window'], m['Engine'] = label, 'V14'
                rows.append(m)
            if v13_mod is not None:
                sub = slice_window(df, start)
                if len(sub) > WARMUP + 100:
                    tr13, eq13, bh13 = v13_mod.run_backtest(
                        sub, v13_mod.V13_CFG, INITIAL_CAPITAL,
                        v13_mod.TRAIL_OVERRIDES_V13.get(name))
                    eq13_w = eq13[eq13.index >= sub.index[WARMUP]]
                    m13 = calculate_metrics(eq13_w, tr13, bh13)
                    if m13:
                        m13.update({'Asset': name, 'Window': label, 'Engine': 'V13',
                                    'Start': sub.index[WARMUP].date(),
                                    'End': sub.index[-1].date(),
                                    'Days': (sub.index[-1] - sub.index[WARMUP]).days,
                                    'BH_MaxDD (%)': bh_drawdown(bh13)})
                        rows.append(m13)
    out = pd.DataFrame(rows)
    w_order = [f'{mo:>2}M' for mo in FIXED_MONTHS] + ['Full']
    out['_w'] = out['Window'].map({w: i for i, w in enumerate(w_order)})
    out = (out.sort_values(['_w', 'Asset', 'Engine'])
              .drop(columns='_w').reset_index(drop=True))
    cols = ['Asset', 'Window', 'Engine', 'Start', 'End', 'Days'] + METRIC_COLS
    return out[cols]
    ...
    for name, df in dfs.items():
        for y in sorted(df.index.year.unique()):
            ...
            if start >= end or start >= df.index[-1]:
                continue
            m = eval_window(name, df, start, end, V14_SELECTED)
            if m and m['Days'] >= 60:
                m['Year'] = y
                rows.append(m)
    if not rows:
...
def grouped_bars(ax, labels, a, b, la, lb, title):
    ...
    ax.bar(x - w / 2, a, w, label=la, color='#2F855A')
...
def plot_report(fixed_df, years_df, roll_df, outfile='v14_period_chart.png'):
    fig, axes = plt.subplots(3, 1, figsize=(14, 12),
                             gridspec_kw={'height_ratios': [1, 1, 1.15]})
    btc_f = fixed_df[(fixed_df.Asset == 'BTC') & (fixed_df.Engine == 'V14')]
    grouped_bars(axes[0], list(btc_f.Window), btc_f['Return (%)'].values,
                 btc_f['B&H (%)'].values, 'Strategy (V14)', 'Buy&Hold',
                 'BTC — fixed windows: V14 vs Buy&Hold (%)')

    btc_y = years_df[years_df.Asset == 'BTC'] if not years_df.empty else pd.DataFrame()
    if not btc_y.empty:
        grouped_bars(axes[1], [str(y) for y in btc_y.Year],
                     btc_y['Return (%)'].values, btc_y['B&H (%)'].values,
                     'Strategy (V14)', 'Buy&Hold',
                     'BTC — independent calendar-year runs: V14 vs Buy&Hold (%)')
    else:
        axes[1].text(0.5, 0.5, 'no yearly data', ha='center', va='center')

    hues = {'BTC': '#2F855A', 'ETH': '#D69E2E', 'SOL': '#2B6CB0'}
    for name, g in roll_df.groupby('Asset'):
        axes[2].plot(g.WinEnd, g['Alpha vs B&H'].values, lw=1.8,
                     color=hues.get(name), label=name)
    axes[2].axhline(0, color='gray', lw=0.9, ls='--')
    axes[2].set_title(f'Rolling {ROLL_MONTHS}M windows — V14 alpha vs Buy&Hold (pp)',
                      fontweight='bold')
...
if __name__ == '__main__':
    print('=' * 78)
    print('📊 V14 PERIOD ROBUSTNESS — Strategy (P2) vs Buy&Hold, multiple windows')
    print('=' * 78)
    dfs = load_all()

    # 1 ── fixed windows -------------------------------------------------
    fixed = run_fixed(dfs, include_v13=True)
    print('\n' + '=' * 78)
    ...
    print('\n' + '=' * 78)
    print('2️⃣  CALENDAR-YEAR RUNS (fresh $1,000 per year) — V14 vs B&H')
    print('=' * 78)
    if not years.empty:
        yshow = years.copy()
    ...
    print('\n' + '=' * 78)
    print(f'3️⃣  ROLLING {ROLL_MONTHS}M WINDOWS (step {ROLL_STEP}M) — SUMMARY (V14)')
    print('=' * 78)
    print(summ.to_string())
    print('\n--- Worst rolling windows (bottom 8 by alpha) ---')
    worst = roll.nsmallest(8, 'Alpha vs B&H')
    ...
    print('\n--- Best rolling windows (top 8 by alpha) ---')
    best = roll.nlargest(8, 'Alpha vs B&H')
    bshow = best[['Asset', 'Start', 'End', 'Return (%)', 'B&H (%)',
                  'Alpha vs B&H', 'MaxDD (%)', 'BH_MaxDD (%)']].copy()
    bshow['Start'] = bshow['Start'].astype(str); bshow['End'] = bshow['End'].astype(str)
    ...
    print('🧠 VERDICT')
    print('=' * 78)
    for name, g in fixed[(fixed.Engine == 'V14')].groupby('Asset'):
        beat = (g['Alpha vs B&H'] > 0).sum()
    ...
    for name, g in roll.groupby('Asset'):
        beat = (g['Alpha vs B&H'] > 0).sum()
        print(f"  • {name}: beat B&H in {beat}/{len(g)} rolling {ROLL_MONTHS}M windows")
    print()

    # ── save artifacts ────────────────────────────────────────────────
    os.makedirs('data', exist_ok=True)
    fixed.to_csv('data/v14_period_fixed.csv', index=False)
    years.to_csv('data/v14_period_years.csv', index=False)
    roll.to_csv('data/v14_period_rolling12m.csv', index=False)
    summ.to_csv('data/v14_period_rolling_summary.csv')
    print('[SAVED] data/v14_period_*.csv')
