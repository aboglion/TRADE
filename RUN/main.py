...
def main() -> None:
    ...
    macro_strategy = RegimeAdaptiveStrategy(
        ...
        mid_leverage=getattr(config.strategy, "mid_leverage", 5.5),
        base_leverage=getattr(config.strategy, "base_leverage", 2.5),
        min_leverage=config.strategy.min_leverage,
        momentum_cutoff_pct=getattr(config.strategy, "momentum_cutoff_pct", -0.018),
        safe_cash_weight=getattr(config.strategy, "safe_cash_weight", 0.75),
        safe_spot_weight=getattr(config.strategy, "safe_spot_weight", 0.18),
        safe_micro_weight=getattr(config.strategy, "safe_micro_weight", 0.07),
