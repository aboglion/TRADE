...
def test_orchestrator_in_cycle_position_flip_margin_and_realized_pnl():
    """Verify that in-cycle position flip calculates exact margin released vs needed and adds realized PnL."""
    mock_config = MagicMock()
    mock_config.run_mode = RunMode.DRY_RUN
    mock_config.exchange.market_type = "future"
    mock_config.risk.min_seconds_between_orders = 0
    ...
    # Initial holding: 1.0 BTC bought at 40,000. Initial USDT: 10,000 free and 10,000 total.
    initial_holding = AssetHolding(
        symbol="BTC",
        free=0.0,
        locked=0.0,
    ...
    portfolio = PortfolioSnapshot(
        timestamp_ms=int(time.time() * 1000),
        holdings={"BTC": initial_holding, "USDT": initial_usdt},
        total_value_usd=50000.0,
    )

    # Flip order: SELL 2.0 BTC @ 60,000 with 2x leverage. Fee is $20 USDT.
    # Long 1.0 is closed -> realized PnL = (60k - 40k) * 1.0 = +20,000. Margin released = 60,000 / 2 = 30,000.
    # Short 1.0 is opened -> margin needed = 60,000 / 2 = 30,000. Net margin delta = 0.
    # Free USDT should be: 10,000 + 0 + 20,000 - 20 = 29,980.
    # Total USDT should be: 10,000 + 20,000 - 20 = 29,980.
    intent = OrderIntent(
        client_order_id="flip_margin_test",
        symbol="BTC/USDT",
        side=OrderSide.SELL,
        order_type=OrderType.MARKET,
        amount=2.0,
    ...
    mock_order_mgr.execute.return_value = OrderResult(
        client_order_id="flip_margin_test",
        exchange_order_id="dry_flip_margin",
        status=OrderStatus.FILLED,
        filled_amount=2.0,
        average_price=60000.0,
        fees=20.0,
    ...
    decision = MagicMock()
    decision.regime = Regime.BEAR
    decision.metadata = {"short_leverage": 2.0, "effective_leverage": 0.0}
    ...
    assert usdt.free == pytest.approx(29980.0, abs=1e-2)
    assert usdt.total == pytest.approx(29980.0, abs=1e-2)


def test_portfolio_service_flip_orders_explicit_leverage():
    """Verify that flip orders generated by PortfolioService carry explicit pos_lev."""
    mock_gw = MagicMock()
    ...
    # Currently Long 1.0 BTC with 3.0x leverage
    ...
    portfolio = PortfolioSnapshot(
        timestamp_ms=int(time.time() * 1000),
        holdings={"BTC": btc_holding, "USDT": AssetHolding("USDT", free=50000.0, locked=0.0, total=50000.0, value_usd=50000.0)},
        total_value_usd=110000.0,
    )
    prices = {"BTC/USDT": 60000.0}

    # Target is Short 30% (-0.30)
    ...
    close_order = next(o for o in plan.orders if "Close previous" in o.reason)
    open_order = next(o for o in plan.orders if "Open new" in o.reason)

    assert close_order.leverage == 3.0
    assert open_order.leverage == 3.0
