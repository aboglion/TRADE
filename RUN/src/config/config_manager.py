...
class StrategyConfig:
    ...
    mid_leverage: float = 5.5
    base_leverage: float = 2.5
    min_leverage: float = 1.0
    momentum_cutoff_pct: float = -0.018
    safe_cash_weight: float = 0.75
    safe_spot_weight: float = 0.18
    safe_micro_weight: float = 0.07
    flash_wick_limit: float = -0.038
    flash_wick_limit_20x: float = -0.022
    atr_20x_limit: float = 0.021
    ladder_steps: list[float] = field(default_factory=lambda: [1.0, 2.5, 5.5, 10.0, 20.0])
...
class ConfigManager:
    ...
    def _load_strategy(self, config: BotConfig, raw: dict) -> None:
        ...
        config.strategy = StrategyConfig(
            ...
            conviction_leverage=s_raw.get("conviction_leverage", 20.0),
            bull_leverage=s_raw.get("bull_leverage", 10.0),
            mid_leverage=s_raw.get("mid_leverage", 5.5),
            base_leverage=s_raw.get("base_leverage", 2.5),
            min_leverage=s_raw.get("min_leverage", 1.0),
            momentum_cutoff_pct=s_raw.get("momentum_cutoff_pct", -0.018),
            safe_cash_weight=s_raw.get("safe_cash_weight", 0.75),
            safe_spot_weight=s_raw.get("safe_spot_weight", 0.18),
            safe_micro_weight=s_raw.get("safe_micro_weight", 0.07),
            flash_wick_limit=s_raw.get("flash_wick_limit", -0.038),
            flash_wick_limit_20x=s_raw.get("flash_wick_limit_20x", -0.022),
            atr_20x_limit=s_raw.get("atr_20x_limit", 0.021),
            ladder_steps=s_raw.get("ladder_steps", [1.0, 2.5, 5.5, 10.0, 20.0]),
