...
class TradingConfig:
    ...
    MARKET_CONDITIONS = {
        'BUY': {
            'relative_strength_threshold': 1,    # כניסה בקניה כאשר RS גבוה
            'order_imbalance_threshold': 0.55,
...
class MarketData:
    def __init__(self, max_size: int = 1000):
        ...
        self.bid_volume: deque = deque(maxlen=500)
        self.ask_volume: deque = deque(maxlen=500)
        self.time_stamps: deque = deque(maxlen=max_size)
        self.high_prices: deque = deque(maxlen=30)  # עבור חישוב ATR
        self.low_prices: deque = deque(maxlen=30)   # עבור חישוב ATR
        self.prev_price: float = 0
        self.round_num: int = 0
        # רשימת היסטורית ערכי "unchanged_price_count" לשם חישוב ממוצע
        self.unchanged_history: deque = deque(maxlen=100)
        self._lock = threading.RLock()  # Thread-safe operation
        
    def add_tick(self, price: float, volume: float, is_ask: bool, timestamp: datetime) -> bool:
        """הוספת נתוני tick עם עיגול נכון ועדכון מעקב"""
        with self._lock:
            if self.round_num == 0 or self.prev_price == 0:
            ...
            round_num = max(0, self.round_num - 1)  # Use one less decimal for significance check
            
            # Check if price change is significant
            if not round(self.prev_price, round_num) == round(price, round_num):
                ...
                return True  # שינוי משמעותי במחיר
            
            # No significant price change, just update volumes
            if is_ask:
                self.ask_volume.append(volume)
            else:
                self.bid_volume.append(volume)
    ...
    def reset(self) -> None:
        ...
        with self._lock:
            ...
            self.prev_price = 0
            self.round_num = 0
            self.unchanged_history.clear()
    
    def get_average_unchanged(self) -> float:
        """חישוב ממוצע ערכי unchanged_price_count מההיסטוריה"""
        with self._lock:
            if self.unchanged_history:
                return sum(self.unchanged_history) / len(self.unchanged_history)
            return 0
        ...
        with self._lock:
            ...
            try:
                ...
                self.metrics.update({
                    ...
                    'market_efficiency_ratio': float(mer)
                })
                ...
                if len(self.market_data.high_prices) >= 14 and len(self.market_data.low_prices) >= 14:
                    ...
                    if len(prices) >= 15:
                        ...
                        if len(closes) >= 14:
                            ...
                            tr1 = highs - lows
                            tr2 = np.abs(highs - closes[:14])
                            tr3 = np.abs(lows - closes[:14])
...
class MarketAnalyzer:
    ...
    def process_tick(self, tick_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            try:
                ...
                significant_change = self.market_data.add_tick(price, volume, is_ask, timestamp)
                if not significant_change:
                    # אין שינוי משמעותי – מעדכנים את המונה
                    self.metrics_calculator.metrics["unchanged_price_count"] += 1
                    return None
                else:
                    # במקרה של שינוי משמעותי, רשמים את המונה בהיסטוריה לפני איפוסו
                    if self.metrics_calculator.metrics["unchanged_price_count"] > 0:
                        self.market_data.unchanged_history.append(self.metrics_calculator.metrics["unchanged_price_count"])
                    self.metrics_calculator.metrics["unchanged_price_count"] = 0
                ...
                # יצירת אות מסחר – כאן נבצע רק תנאי כניסה (Long) תוך שילוב unchanged_price_count
    ...
    def _signal_generator(self, price: float, timestamp: datetime) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # תנאי כניסה ל-Long – נוסיף את תנאי unchanged_price_count:
            avg_unchanged = self.market_data.get_average_unchanged()
            
            # Buy entry conditions
            buy_entry = (
                metrics['relative_strength'] >= conditions['BUY']['relative_strength_threshold'] and
                metrics['order_imbalance'] > conditions['BUY']['order_imbalance_threshold'] and
                metrics['realized_volatility'] < conditions['BUY']['volatility_threshold'] and
                metrics['trend_strength'] > conditions['BUY']['trend_confirmation'] and
                metrics['unchanged_price_count'] < (avg_unchanged / 1.15 if avg_unchanged > 0 else float('inf'))
            )
    ...
    def _close_position(self, exit_price: float, reason: str, timestamp: datetime) -> None:
        ...
        try:
            ...
            journal_record['notes'] = (
                ...
                f"Trend: {self.metrics_calculator.metrics['trend_strength']:.2f}"
            )
            ...
            try:
                ...
                if need_new_connection:
                    ...
                    self.ws_connections[symbol].run_forever(
                        ping_interval=30,
                        ping_timeout=10,
                        reconnect=5  # Automatic reconnect with 5 second delay
                    )
...
class MarketStatusReporter:
    ...
    def __init__(self, analyzer: MarketAnalyzer, report_interval: int = 30):
        self.analyzer = analyzer
